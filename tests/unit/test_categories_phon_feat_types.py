"""Unit tests for phon_feat_types leaf-category functions
(coverage-content-fidelity-v2 Part B sub-part 4, final).

Structurally IDENTICAL to test_categories_feature_struct_types.py (Part B.2)
but under the PHONOLOGICAL feature system (PhFeatureSystemOA rather than
MsFeatureSystemOA). PHON_FEAT_TYPES is classified GOLD_RESERVED in models.py
(unlike its MULTI_INSTANCE sibling FEATURE_STRUCT_TYPES) -- see (e) below for
the conflict-mode assertion through the same API test_conflict_mode_model.py
uses.

Covers:
- enumerate_source() walks source PhFeatureSystemOA.TypesOC.
...
- execute_action():
  (a) absent-in-target IFsFeatStrucType -> CREATE, GUID-preserved, landed
      in PhFeatureSystemOA.TypesOC; Name/Abbreviation/Description
      ws-mapped-copied.
  (b) already-present-by-GUID handled at plan_action (no duplicate CREATE
      attempted for a GUID already in target TypesOC).
  (c) FeaturesRS member whose GUID resolves in target PhFeatureSystemOA
      .FeaturesOC -> FeaturesRS.Add() called; a member with NO target
      counterpart is skipped + logged, no crash, partial wiring tolerated.
- registry test (test_category_registry.py) passes with PHON_FEAT_TYPES
  present -- exercised indirectly via `categories.for_category`.
- (e) conflict-mode: PHON_FEAT_TYPES resolves to ConflictMode.UPDATE via the
  GOLD_RESERVED path (Selection.conflict_mode_for), the same API
  test_conflict_mode_model.py uses.
"""
...
class _FakeLangProject:
    def __init__(self, feature_system) -> None:
        self.PhFeatureSystemOA = feature_system
...
_BUNDLE = categories.for_category(GrammarCategory.PHON_FEAT_TYPES)
...
def test_enumerate_source_returns_all_types() -> None:
    ...
    tgt = _FakeProject()
    ctx = _ctx(src, tgt)
    sel = Selection(categories={GrammarCategory.PHON_FEAT_TYPES: True})
...
def test_plan_action_new_guid_yields_planned_action() -> None:
    ...
    assert result.category == GrammarCategory.PHON_FEAT_TYPES
...
def test_execute_action_creates_type_lands_in_types_oc(_patch_lcm) -> None:
    """(a) absent-in-target -> CREATE, GUID-preserved, landed in
    PhFeatureSystemOA.TypesOC."""
    fake_lcm = _patch_lcm
    ...
    result = categories.phon_feat_types_execute_action(
    ...
    assert new_type in feat_sys.TypesOC, "new type was not landed in PhFeatureSystemOA.TypesOC"
...
def test_execute_action_features_rs_member_resolved_is_wired(_patch_lcm) -> None:
    """(c) a FeaturesRS member whose GUID resolves in target
    PhFeatureSystemOA.FeaturesOC -> FeaturesRS.Add() called with the TARGET
    defn object."""
    fake_lcm = _patch_lcm
...
# PHON_FEAT_TYPES-specific double-check kept local for fast failure
...
def test_for_category_dispatch_returns_same_bundle() -> None:
    assert categories.for_category(GrammarCategory.PHON_FEAT_TYPES) is _BUNDLE


# ============================================================================
# (e) Conflict-mode: PHON_FEAT_TYPES resolves to ConflictMode.UPDATE via the
# GOLD_RESERVED path -- same API (Selection.conflict_mode_for) that
# test_conflict_mode_model.py exercises for the other GOLD_RESERVED
# categories. This is the "categorical tension" flag: PHON_FEAT_TYPES is
# GOLD_RESERVED while its structurally-identical sibling FEATURE_STRUCT_TYPES
# is MULTI_INSTANCE; both land on UPDATE under v7.0.0, so this test locks the
# OBSERVABLE behavior (not the internal classification bucket).
# ============================================================================

def test_conflict_mode_for_phon_feat_types_is_update_via_gold_reserved() -> None:
    sel = Selection()
    assert sel.conflict_mode_for(GrammarCategory.PHON_FEAT_TYPES) == ConflictMode.UPDATE
