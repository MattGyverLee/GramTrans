...
- [X] T029 [US3] Implement `reproduce_allomorph_hung_data(src_allo, new_allo, ctx, tag, resolver_cache, dropped)` in `src/gramtrans/Lib/owned.py`: resolve `PhoneEnvRC` via the resolver (link/report, do not create environments here); discover APRs in `MorphologicalDataOA.AdhocCoProhibitionsOC` whose `FirstAllomorphRA`/`RestOfAllosRS`/`AllomorphsRS` reference a copied allomorph/morpheme and reproduce only all-members-in-copy-set, else report (FR-009a, research R6; depends on T027, T015).
  - **Done (this cycle)**: `PhoneEnvRC` links/reports against `lp.PhonologicalDataOA.EnvironmentsOS` (a flat owned SEQUENCE, confirmed live -- NOT under `LexDbOA`, NOT routed through `references._find_in_possibility_list`; environments are never created here). `StemNameRA` resolves against the OWNING POS's own `StemNamesOC` (POS resolved via the existing `categories._resolve_target_pos` idiom, reused via lazy import) or REPORTs (never creates a StemName). APRs: discovered from source `MorphologicalDataOA.AdhocCoProhibitionsOC` by scanning `FirstAllomorphRA`/`RestOfAllosRS`/`AllomorphsRS` for a reference to the allomorph currently being copied; reproduced (via `IMoAlloAdhocProhibFactory.Create(guid)` + manual `AdhocCoProhibitionsOC.Add`, the UNOWNED_THEN_ADD shape applied directly since the owner is `LangProject.MorphologicalDataOA` not a per-child `OwnedObjectSpec` row) only when every member's GUID is in the run's `ctx._copy_set`, else one `DroppedItemRecord` ("member not in copy set") per missing member and the APR is NOT created; dedup via a `resolver_cache`-held set of already-reproduced APR GUIDs (guards against double-creation across multiple allomorphs of the same APR in one run). `ctx._copy_set` (new `dict[guid, copied_target_obj]` RunContext convention, threaded like `_dropped`/`_resolver_cache`) is populated by the caller (T030) with the allomorph currently being processed before this call. QC cleanup: removed the dead `owning_fields` param from `_matches_owner_class` (was never read in its body). `tests/unit/test_allomorph_hung_data.py`'s 7 tests (6 write-first RED + 1 pre-existing GREEN guard) all pass.
- [X] T030 [US3] Invoke `walk_owned_children` (from the entry/sense closure) and `reproduce_allomorph_hung_data` (from the allomorph copy) in `src/gramtrans/Lib/categories.py`, and surface owned-object decisions in the Preview plan / execute in move (`preview.py`/`transfer.py`, Principle III; depends on T028, T029, T017).
  - **owned-CHILD portion DONE (prior cycle)**: `categories._walk_lex_entry_closure` now calls `owned.walk_owned_children` twice -- ENTRY level (`owning_fields={"PronunciationsOS", "EtymologyOS"}`) and SENSE level (unfiltered -- `ExamplesOS` + recursive sub-senses via `OwnedObjectSpec.recurse`). The `owning_fields` filter (new optional kwarg on `walk_owned_children`, default `None` = unfiltered, back-compat with all 5 pre-existing `test_owned_object_walk.py` calls) exists specifically to stop the entry-level call from ALSO matching `OWNED_OBJECT_MAP`'s `LexSense.SensesOS` row -- a real `ILexEntry` duck-types a `SensesOS` attribute too (its own top-level senses), which would otherwise re-create every top-level sense a second time as a phantom owned child, double-processing the senses `_walk_lex_entry_closure`'s own `for src_sense in src_entry.SensesOS` loop already creates directly. Preview-side: `owned.plan_owned_object_decisions` (+ `_plan_child_ref_decisions` / `_plan_full_sense_reference_decisions`), a read-only twin of the write walk (decide-only, never creates/writes), wired the same way (entry-level filtered, sense-level unfiltered) into `categories._plan_entry_reference_decisions`, appending to the same `PlannedAction.reference_decisions` tuple the top-level entry/sense/allomorph decisions already populate.
  - **allomorph-HUNG-DATA portion DONE (this cycle)**: `categories._walk_entry_allomorphs._mk` now records the just-created allomorph into `context._copy_set[src_guid] = new_allo` (mirrors `identity_remap`'s per-allomorph bookkeeping) then calls `owned.reproduce_allomorph_hung_data(...)` right where the `# TODO(024 cycle-10)` marker was -- PhoneEnvRC/StemNameRA/APR now actually get reproduced/reported at Move time. Preview twin: `categories._plan_entry_reference_decisions`'s allomorph loop records the allomorph's GUID into `context._copy_set` (marker `True`, no real target object exists yet in Preview) and calls `owned.plan_allomorph_hung_data_decisions(...)`, extending the same `records` tuple `PlannedAction.reference_decisions` already carries. `ctx._copy_set` is initialized once per run alongside `_dropped`/`_resolver_cache`: `preview.build_run_plan` and `transfer.execute` both now `object.__setattr__` an empty dict onto (`exec_`)`context` before the leaf-dispatch loop, so the copy set accumulates across every entry in the SAME run (not reset per-entry) -- matching Move's actual progressive-discovery semantics; `categories.py` also keeps a defensive lazy-init fallback (mirrors `_get_resolver_cache`) in case a caller hasn't threaded it yet. Full `tests/unit/` suite: 1440 passed, 4 failed (1 known-unrelated `test_wizard_pos_grammar_wiring.py` + 3 pre-existing `test_lexical_relations.py` RED tests, T031, out of this cycle's scope), 9 skipped, 14 xfailed, 14 xpassed -- identical to pre-change baseline for everything outside T029's own 6 newly-green tests.
- [X] T031 [US3] Reproduce lexical relations for copied members only (preserve mapping/tree/pair structure; report members not in the copy set) in `src/gramtrans/Lib/categories.py` via the resolver, reusing residue's registered `LexReference` carrier (FR-008; depends on T016).
  - **Done (this cycle)**: `categories._evaluate_lexical_relation` resolves the target `ILexRefType` by GUID via `references._find_in_possibility_list` against `target.Cache.LangProject.LexDbOA.ReferencesOA` (possibility-list-shaped -- reused verbatim), classifies every `TargetsRS` member against `ctx._copy_set`, and applies the FR-008 partial-member policy using the MCP-confirmed real `LexRefTypeTags.MappingTypes` int values (PAIR-family {1,2,6,7,11,12}: below-2-copied-members is incoherent, relation dropped whole with ONE relation-keyed `DroppedItemRecord`, no per-member reports; TREE-family {3,8,13}: `TargetsRS[0]` (root) not copied -> whole relation dropped the same way; COLLECTION/SEQUENCE/UNIDIRECTIONAL: reproduced with whatever subset was copied, each non-copied member reported individually, never silently included/dropped). `reproduce_lexical_relation(src_relation, ctx, tag, resolver_cache, dropped)` (Move) creates via the confirmed-live OWNER-TAKING `ILexReferenceFactory.Create(guid, targetLexRefType)` (factory itself adds to `MembersOC`), populates `TargetsRS` with ONLY the copied members preserving order, tags via `residue.apply_residue(..., class_name="LexReference")` (Carrier A, already registered). GUID-keyed dedup in `resolver_cache` -- a successful creation is cached and returned on a later call for the same relation guid; a failed/incoherent attempt is NOT cached (mirrors `owned._reproduce_one_apr`'s retry posture -- a missing member now might be copied by a later call). `plan_lexical_relation_decision` is the read-only Preview twin (same `_evaluate_lexical_relation` core, never creates, returns a `ReferenceDecisionRecord` action=CREATE). Discovery (`_discover_lex_relations_for_member`, scans every `ILexRefType` in the source's `ReferencesOA` + each type's `MembersOC` for a `TargetsRS` match -- mirrors `owned._source_aprs`'s scan-and-match pattern) is invoked per copied entry/sense from `_walk_lex_entry_closure` (Move, `_reproduce_lex_relations_for_member`) and `_plan_entry_reference_decisions` (Preview, `_plan_lex_relations_for_member`), right after each is registered into `context._copy_set` (entry/sense guids now added to that dict alongside the pre-existing allomorph guids). Test-file correction: the write-first fixtures' `_MAPPING_TYPE_COLLECTION`/`_MAPPING_TYPE_PAIR` placeholder sentinels (100/200) were replaced with the real enum values (10 `kmtEntryOrSenseCollection`, 11 `kmtEntryOrSensePair`) per MCP confirmation -- a legitimate correction, not test-weakening (docstring updated to record the mapping). `tests/unit/test_lexical_relations.py`'s 3 write-first RED tests now pass; full `tests/unit/` suite: 1443 passed (1440 baseline + these 3), 1 failed (same known-unrelated `test_wizard_pos_grammar_wiring.py`), 9 skipped, 14 xfailed, 14 xpassed -- baseline preserved. Flag for T037 live verification: the discovery/wiring leg (entry/sense-level relation discovery + Preview surfacing) has no live-MCP or integration-test coverage yet, only the per-relation `reproduce_lexical_relation`/`plan_lexical_relation_decision` core is unit-tested against fakes.
  - **QC P1 fix (follow-up cycle, FR-009 sub-sense copy_set accuracy gap, commit 92d92fa write-first RED)**: `owned.py`'s recursive `SensesOS` leg (`_copy_one_owned_child`'s `if spec.recurse:` branch, Move; `plan_owned_object_decisions`'s matching `if spec.recurse:` branch, Preview) never registered the newly-created sub-sense into `ctx._copy_set` -- only the entry/top-level-sense registration sites did. Fixed via a new `owned._register_copy_set(ctx, guid, value)` helper (source sub-sense guid -> new target sub-sense on Move, -> `True` placeholder on Preview, matching the existing convention) called immediately after `_apply_full_sense_reference_fields`/`_plan_full_sense_reference_decisions`, PLUS a matching lexical-relation discovery call for the sub-sense's own GUID -- `owned._reproduce_lex_relations_for_recursed_child` (Move, calls `categories._reproduce_lex_relations_for_member`) and `owned._plan_lex_relations_for_recursed_child` (Preview, calls `categories._plan_lex_relations_for_member`) -- mirroring the top-level entry/sense wiring in `_walk_lex_entry_closure`/`_plan_entry_reference_decisions`. Fires exactly once per created sub-sense (from `_copy_one_owned_child`, never from the unconditional owned-collection re-walk); `reproduce_lexical_relation`'s pre-existing GUID-keyed dedup in `resolver_cache` makes a later, independent discovery call for the SAME relation (e.g. via the sub-sense's owning top-level sense) idempotent, so no relation is ever double-created and no infinite recursion risk (discovery never re-enters `walk_owned_children`). Also fixed (QC P2): `categories._walk_entry_allomorphs._mk`'s copy_set late-init fallback (~line 4128) was missing the `object.__setattr__` persistence the other three registration sites use -- a late-initialized dict there was silently thrown away after the function returned; now persists consistently. APR gate, pair<2 drop-whole ruling, and tree/collection rulings are UNCHANGED (APR members are allomorphs, not senses -- no interaction with this fix). `tests/unit/test_subsense_copy_set.py`'s 2 write-first RED tests now pass; full `tests/unit/` suite: 1447 passed (1445 baseline + these 2), 1 failed (same known-unrelated `test_wizard_pos_grammar_wiring.py`), 9 skipped, 14 xfailed, 14 xpassed -- baseline preserved.
...
## Delivery Status (2026-07-12 — 024 complete, authoritative)

This summary supersedes the per-task inline notes above where they diverge (in
particular the T031 inline note describes an early incremental-discovery approach that
was later replaced — see below). Delivered across implementation cycles (LEX-crew
dispatch, TDD write-first).

- **US1–US5: DONE.** Full suite green (1494 passed; 1 pre-existing unrelated wizard/POS
  failure excepted). Merged to `main` at T038 this session.
- **T031 (lexical relations): DONE — as-built supersedes the inline note.** Shipped as a
  **single final pass** at closure-end (`reproduce_all_lexical_relations` /
  `plan_all_lexical_relations`), source-ordered by construction, per-`MappingType`
  rulings, copied-members-only. The earlier per-member incremental discovery
  (`_discover_lex_relations_for_member` etc., described in the T031 inline note) was
  **removed** in the cycle-14 rework — no incremental trigger remains. FR-008 wording
  matches the delivered behavior.
- **T032–T034 (census, FR-011): DONE** — `tests/verification/fidelity_census.py`, 75
  fields / 11 classes, four terminal buckets (COPIED / DROP_REPORTED / HANDLED_ELSEWHERE /
  OUT_OF_SCOPE_EXCLUDED), never-silent guard (raises on unclassified). The census surfaced
  a real, previously-silent gap (variant `LexEntryRef` relationships) and forced a
  mid-implementation **exclusion-policy correction** — see the "Delivery Status & As-Built
  Amendments" section of `spec.md`. Deferred reproductions routed to stubs **027–030**.
- **T035 (SC-006 no-regression): DONE in spirit** — the dropped-item report / census suite
  covers "no-custom-lists → empty dropped-items"; SC-006 holds.
- **T036 (docs): code-side DONE** (module docstrings updated). No repo `docs/` directory
  exists, so the `docs/ARCHITECTURE` portion is N/A for this repo.
- **T037 (live GUI quickstart, Scenarios 1–5): DEFERRED** — fixture-dependent. All paths
  are unit/fakes-proven; several are vacuous-live on Ejagham Mini and need constructed
  fixtures for end-to-end live proof. See [validation-status.md](./validation-status.md).
- **T038 (full suite green + merge): DONE** this session.
