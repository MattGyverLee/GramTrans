...
is the complete set. All twenty-two are additive, textual, or pure relocation, and
none alters a FlexTools code path's behaviour. Rows 11-21 were added by feature 039
(the `selection_wizard.py` module split); rows 1-10 are feature 034's own.
...
| 11 | `src/gramtrans/Lib/ui/wizard_roles.py` | **New file**: every `Qt.ItemDataRole.UserRole + N` item-data role constant in one collision-free block, plus `_STATUS_LABELS`, `_RULES_STATUS_LABELS`, `_CF_LEVEL_LABELS`. | Feature 039 splits the 6512-line `selection_wizard.py` into page modules. FR-003/SC-008 there cap any one `Lib/ui/` module at 1750 lines, and the monolith's size was actively concealing defects (two item-data role blocks collided on `UserRole + 70..72`; a recursive generator was called and discarded). The split cannot happen without new files under `Lib/`, and `check_shared_exceptions.py` counts an addition as a change. | **Pure relocation** — the code is byte-identical to the hunks it left `selection_wizard.py` as, and `selection_wizard.py` eagerly imports and re-exports every name so `selection_wizard.X` still resolves for both the FlexTools flat-import path and the frozen bundle. Nothing on the FlexTools path imports these modules by name; the only production import of the wizard remains `SelectionWizard` at `gramtrans.py:249`. Both branches of the `if __package__:` dual-mode guard are present (feature 039 FR-007), so the flat `site.addsitedir` path resolves identically. Constants only — no Qt widget construction, no I/O. |
| 12 | `src/gramtrans/Lib/ui/wizard_page_base.py` | **New file**: `_FlowPage` (relocated verbatim; it has zero external references) plus the shared bases `_ProjectHandlesMixin`, `_PickDerivedMixin` and `_BlockPage` that absorb the ~505 measured duplicate lines. | Feature 039 splits the 6512-line `selection_wizard.py` into page modules. FR-003/SC-008 there cap any one `Lib/ui/` module at 1750 lines, and the monolith's size was actively concealing defects (two item-data role blocks collided on `UserRole + 70..72`; a recursive generator was called and discarded). The split cannot happen without new files under `Lib/`, and `check_shared_exceptions.py` counts an addition as a change. | **Pure relocation** — the code is byte-identical to the hunks it left `selection_wizard.py` as, and `selection_wizard.py` eagerly imports and re-exports every name so `selection_wizard.X` still resolves for both the FlexTools flat-import path and the frozen bundle. Nothing on the FlexTools path imports these modules by name; the only production import of the wizard remains `SelectionWizard` at `gramtrans.py:249`. Both branches of the `if __package__:` dual-mode guard are present (feature 039 FR-007), so the flat `site.addsitedir` path resolves identically. The bases hold one copy of methods that were already byte-identical across 8 page classes, so every page keeps the exact behaviour it had. |
| 13 | `src/gramtrans/Lib/ui/wizard_widgets.py` | **New file**: the wizard's widget/tree helpers — `_page_progress`, `_source_counts_of`, `_operation_failed_note`, `_show_failure_row`, `_make_tree_pane_splitter`, `_item_views_of`, `_elide_over_narrow_columns`, `_set_item_text_with_tooltip`, `_carry_full_values_in_tooltips`, `_make_group_item`, `_count_affixes_in_node`, `_TREE_PANE_MIN_WIDTH`, `_PREVIEW_PANE_MIN_WIDTH`. | Feature 039 splits the 6512-line `selection_wizard.py` into page modules. FR-003/SC-008 there cap any one `Lib/ui/` module at 1750 lines, and the monolith's size was actively concealing defects (two item-data role blocks collided on `UserRole + 70..72`; a recursive generator was called and discarded). The split cannot happen without new files under `Lib/`, and `check_shared_exceptions.py` counts an addition as a change. | **Pure relocation** — the code is byte-identical to the hunks it left `selection_wizard.py` as, and `selection_wizard.py` eagerly imports and re-exports every name so `selection_wizard.X` still resolves for both the FlexTools flat-import path and the frozen bundle. Nothing on the FlexTools path imports these modules by name; the only production import of the wizard remains `SelectionWizard` at `gramtrans.py:249`. Both branches of the `if __package__:` dual-mode guard are present (feature 039 FR-007), so the flat `site.addsitedir` path resolves identically. |
| 14 | `src/gramtrans/Lib/ui/wizard_page_projects.py` | **New file**: `_PageProjects` — wizard step 1, the source/target selection page (including the `_PageProjectWS` legacy alias the suite probes for). | Feature 039 splits the 6512-line `selection_wizard.py` into page modules. FR-003/SC-008 there cap any one `Lib/ui/` module at 1750 lines, and the monolith's size was actively concealing defects (two item-data role blocks collided on `UserRole + 70..72`; a recursive generator was called and discarded). The split cannot happen without new files under `Lib/`, and `check_shared_exceptions.py` counts an addition as a change. | **Pure relocation** — the code is byte-identical to the hunks it left `selection_wizard.py` as, and `selection_wizard.py` eagerly imports and re-exports every name so `selection_wizard.X` still resolves for both the FlexTools flat-import path and the frozen bundle. Nothing on the FlexTools path imports these modules by name; the only production import of the wizard remains `SelectionWizard` at `gramtrans.py:249`. Both branches of the `if __package__:` dual-mode guard are present (feature 039 FR-007), so the flat `site.addsitedir` path resolves identically. Row 8's `source_binder=None` behaviour moves with the class unchanged, so the FlexTools path still builds no source button. |
| 15 | `src/gramtrans/Lib/ui/wizard_page_ws.py` | **New file**: `_PageWritingSystems` plus `_enumerate_active_ws_ids` and `_enumerate_ws_by_kind`. | Feature 039 splits the 6512-line `selection_wizard.py` into page modules. FR-003/SC-008 there cap any one `Lib/ui/` module at 1750 lines, and the monolith's size was actively concealing defects (two item-data role blocks collided on `UserRole + 70..72`; a recursive generator was called and discarded). The split cannot happen without new files under `Lib/`, and `check_shared_exceptions.py` counts an addition as a change. | **Pure relocation** — the code is byte-identical to the hunks it left `selection_wizard.py` as, and `selection_wizard.py` eagerly imports and re-exports every name so `selection_wizard.X` still resolves for both the FlexTools flat-import path and the frozen bundle. Nothing on the FlexTools path imports these modules by name; the only production import of the wizard remains `SelectionWizard` at `gramtrans.py:249`. Both branches of the `if __package__:` dual-mode guard are present (feature 039 FR-007), so the flat `site.addsitedir` path resolves identically. |
| 16 | `src/gramtrans/Lib/ui/wizard_pages_pickers.py` | **New file**: the Model-A item-derived picker pages `_PageItemPicker` and `_PageStemPicker`. | Feature 039 splits the 6512-line `selection_wizard.py` into page modules. FR-003/SC-008 there cap any one `Lib/ui/` module at 1750 lines, and the monolith's size was actively concealing defects (two item-data role blocks collided on `UserRole + 70..72`; a recursive generator was called and discarded). The split cannot happen without new files under `Lib/`, and `check_shared_exceptions.py` counts an addition as a change. | **Pure relocation** — the code is byte-identical to the hunks it left `selection_wizard.py` as, and `selection_wizard.py` eagerly imports and re-exports every name so `selection_wizard.X` still resolves for both the FlexTools flat-import path and the frozen bundle. Nothing on the FlexTools path imports these modules by name; the only production import of the wizard remains `SelectionWizard` at `gramtrans.py:249`. Both branches of the `if __package__:` dual-mode guard are present (feature 039 FR-007), so the flat `site.addsitedir` path resolves identically. |
| 17 | `src/gramtrans/Lib/ui/wizard_pages_skeleton.py` | **New file**: `_PageSkeleton` and `_PageGramDeps`, both derived from the affix/stem picks. | Feature 039 splits the 6512-line `selection_wizard.py` into page modules. FR-003/SC-008 there cap any one `Lib/ui/` module at 1750 lines, and the monolith's size was actively concealing defects (two item-data role blocks collided on `UserRole + 70..72`; a recursive generator was called and discarded). The split cannot happen without new files under `Lib/`, and `check_shared_exceptions.py` counts an addition as a change. | **Pure relocation** — the code is byte-identical to the hunks it left `selection_wizard.py` as, and `selection_wizard.py` eagerly imports and re-exports every name so `selection_wizard.X` still resolves for both the FlexTools flat-import path and the frozen bundle. Nothing on the FlexTools path imports these modules by name; the only production import of the wizard remains `SelectionWizard` at `gramtrans.py:249`. Both branches of the `if __package__:` dual-mode guard are present (feature 039 FR-007), so the flat `site.addsitedir` path resolves identically. |
| 18 | `src/gramtrans/Lib/ui/wizard_pages_blocks.py` | **New file**: the four Model-B independent-block pages `_PageCustomFields`, `_PageRules`, `_PagePhonology`, `_PageEntryTypes`, rebased on `_BlockPage`. | Feature 039 splits the 6512-line `selection_wizard.py` into page modules. FR-003/SC-008 there cap any one `Lib/ui/` module at 1750 lines, and the monolith's size was actively concealing defects (two item-data role blocks collided on `UserRole + 70..72`; a recursive generator was called and discarded). The split cannot happen without new files under `Lib/`, and `check_shared_exceptions.py` counts an addition as a change. | **Pure relocation** — the code is byte-identical to the hunks it left `selection_wizard.py` as, and `selection_wizard.py` eagerly imports and re-exports every name so `selection_wizard.X` still resolves for both the FlexTools flat-import path and the frozen bundle. Nothing on the FlexTools path imports these modules by name; the only production import of the wizard remains `SelectionWizard` at `gramtrans.py:249`. Both branches of the `if __package__:` dual-mode guard are present (feature 039 FR-007), so the flat `site.addsitedir` path resolves identically. One deliberate behaviour change is carried here and is its own revertible commit: `_PageEntryTypes._iter_item_rows` now yields from its recursive walk instead of discarding it, so rows under a nested group are counted. That is a wizard-preview counting fix with no effect on what is written. |
| 19 | `src/gramtrans/Lib/ui/wizard_page_texts.py` | **New file**: `_PageTexts`. | Feature 039 splits the 6512-line `selection_wizard.py` into page modules. FR-003/SC-008 there cap any one `Lib/ui/` module at 1750 lines, and the monolith's size was actively concealing defects (two item-data role blocks collided on `UserRole + 70..72`; a recursive generator was called and discarded). The split cannot happen without new files under `Lib/`, and `check_shared_exceptions.py` counts an addition as a change. | **Pure relocation** — the code is byte-identical to the hunks it left `selection_wizard.py` as, and `selection_wizard.py` eagerly imports and re-exports every name so `selection_wizard.X` still resolves for both the FlexTools flat-import path and the frozen bundle. Nothing on the FlexTools path imports these modules by name; the only production import of the wizard remains `SelectionWizard` at `gramtrans.py:249`. Both branches of the `if __package__:` dual-mode guard are present (feature 039 FR-007), so the flat `site.addsitedir` path resolves identically. |
| 20 | `src/gramtrans/Lib/ui/wizard_pages_deferred.py` | **New file**: `_PageScopeConflict` — constructed for back-compat but absent from `flow()` — plus its exclusive constants `_allowed_modes`, `_CONFLICT_LABELS`, `_SCOPE_LABELS`, `_SCHEMA_CATEGORIES`, `_GOLD_RESERVED`, `_CUSTOM_FIELDS_ONLY`, `_CATEGORY_TOGGLES`. | Feature 039 splits the 6512-line `selection_wizard.py` into page modules. FR-003/SC-008 there cap any one `Lib/ui/` module at 1750 lines, and the monolith's size was actively concealing defects (two item-data role blocks collided on `UserRole + 70..72`; a recursive generator was called and discarded). The split cannot happen without new files under `Lib/`, and `check_shared_exceptions.py` counts an addition as a change. | **Pure relocation** — the code is byte-identical to the hunks it left `selection_wizard.py` as, and `selection_wizard.py` eagerly imports and re-exports every name so `selection_wizard.X` still resolves for both the FlexTools flat-import path and the frozen bundle. Nothing on the FlexTools path imports these modules by name; the only production import of the wizard remains `SelectionWizard` at `gramtrans.py:249`. Both branches of the `if __package__:` dual-mode guard are present (feature 039 FR-007), so the flat `site.addsitedir` path resolves identically. The page is not in `flow()`, so no host reaches it. |
| 21 | `src/gramtrans/Lib/debuglog.py` | Extend the hand-written `LOGGER_NAMES` tuple with the ten new bare module stems alongside the existing `"selection_wizard"`. | Feature 039 splits the 6512-line `selection_wizard.py` into page modules. FR-003/SC-008 there cap any one `Lib/ui/` module at 1750 lines, and the monolith's size was actively concealing defects (two item-data role blocks collided on `UserRole + 70..72`; a recursive generator was called and discarded). The split cannot happen without new files under `Lib/`, and `check_shared_exceptions.py` counts an addition as a change. | Additive: `LOGGER_NAMES` only decides which loggers `configure()` attaches the debug handler to. Under flat import `logging.getLogger(__name__)` yields the bare stem, so **without** these entries a new module's `_module_log` output would go unconfigured in exactly the FlexTools/frozen host this table protects. No existing entry changes and no logging threshold moves. |
| 22 | `src/gramtrans/Lib/ui/ws_font_delegate.py` | Wrap the existing `from ..ws_fonts import LabelRun, WsFont, WsFontRegistry, WsRole` in the `if __package__:` / `else:` dual-mode guard that every other module in `Lib/ui/` already carries. No other change. | Feature 039 SC-006 is "the wizard imports **and constructs** in flat mode", and it could not hold: this was the only file in `Lib/ui/` with an unguarded relative import, and `selection_wizard.py` imports it unconditionally in **both** branches of its own guard -- so a flat load of the wizard failed outright with "attempted relative import with no known parent package". Flat loading is the FlexTools and frozen-bundle path that constitution v5.1.0 mandates (`site.addsitedir(r"Lib")`, constitution.md:380-384), which makes this the same class of defect as the rows above rather than a style preference. | **Pre-existing, and verified so**: the identical failure reproduces on `main` before feature 039 touched anything, so the split neither caused nor worsened it. The dotted branch is byte-identical to the line it replaces, so the package/pytest path is unchanged; the `else` branch is only ever taken where the old code could not run at all. |
