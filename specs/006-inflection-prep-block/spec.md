...
3. **Given** source has "Noun class" on LexEntry and target already has "Noun class" on LexEntry, **When** the linguist transfers Custom Fields, **Then** the run report emits `Skip(ALREADY_PRESENT_BY_GUID)` for that field and zero `NEEDS_MANUAL` notices.
