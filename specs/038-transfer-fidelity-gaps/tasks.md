...
- [ ] **T024c** [US2] **T024's live pairs never run a transfer -- so they cannot sanity-check one.** Raised by the user 2026-08-19 and confirmed by measurement. `_t024_census` (`test_object_census.py:2297`) censuses two projects **as they happen to sit on disk**: it invokes `cli_exit(["run", "--source", ..., "--destination", ..., "--baseline", <starter>, "--destination-freshly-created"])` and nothing anywhere in the 3702-line file restores a target or executes a transfer (`transfer_run` appears only inside the synthetic `make_artifact` fixtures). The destination's pre-transfer state is therefore **unknown and unrecoverable**, and the starter baseline is being used to *model* a prior state instead of measuring it. Three consequences, in ascending severity:
  1. **The `--destination-freshly-created` declaration is false for `Ngoreme Target`.** Measured on disk 2026-08-19: 1415 `LexEntry`, 1552 `LexSense`, 1534 `MoStemAllomorph`, 147 `MoAffixAllomorph`, 50 `Text`, 2271 `Segment`, 64 `PhPhoneme`. That is not a fresh project. `contracts/fidelity-census.md` 5.1 ("Mis-declared") says a `starter_capture` baseline used against a destination the operator did not truthfully declare freshly created is **`CENSUS_ERROR`** -- T024 asserts its way straight through the one condition the contract forbids, and the user's recollection that `Ngoreme Target` "was blank, ready for copying" is exactly the stale premise 5.1 exists to catch.
  2. **The measured deltas are not attributable to any run.** `MoStemMsa` 1949->0 is real arithmetic, but nothing ties it to a transfer this repo controls: it is source-minus-whatever-was-left-behind by an unrecorded earlier session. As evidence of a GramTrans defect it is unfalsifiable and non-reproducible; re-running the suite next week measures a different, equally unattributable delta if anyone touches either project.
  3. **It is the direct cause of T024b.** Because the baseline is a `starter_capture` with `carries_natural_keys: false`, every row lands on `baseline_gross`, 5.2 caps the run verdict at `CENSUS_ACCOUNTED`, and both pairs exit 0 while carrying 44-47 failing rows. A `pre_transfer_census` baseline is exact by construction and carries keys, so doing this correctly **dissolves most of T024b** rather than needing the gate redesign T024b contemplates.

  **The correct shape, which the code already supports end to end:** restore a throwaway target (`tests/integration/harness/restore.py`, per CLAUDE.md write to `Target`, never `Esperanto`) -> `census capture-baseline --project Target` to produce a `pre_transfer_census` (`census_cli.py:1237`) -> run the transfer (`tests/integration/harness/full_run.py`) -> `census run --baseline <pre-transfer>` -> the delta is now attributable, exact, and reproducible from a backup. **Blocked on a backup:** `backups/` holds only `Ejagham Full`, `Ejagham Mini`, and `Target 2026-07-06 0218.fwbackup` -- there is **no `Ngoreme Target` backup**, so the Ngoreme pair cannot be reconstituted and must either be re-derived by transferring `Ngoreme FLEx` into a restored `Target` or dropped. Keep the starter-capture baseline work (T023/T023b): knowing what ships with a blank project stays independently useful, and it remains the right basis for a genuinely fresh destination -- it is just not a substitute for measuring the destination you are about to write to - `tests/integration/test_object_census.py`, `tests/integration/harness/`
