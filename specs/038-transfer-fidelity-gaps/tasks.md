...
- [ ] **T024e** [US2] **The automatic blank filter is UNCONDITIONAL and UNOBSERVABLE -- "force all" does not include blanks, and nothing records what it dropped.** Raised by the user 2026-08-19 (wanting to test the automatic filters as a mode distinct from force-all) and measured the same day. Two facts, both verified:
  1. **There is no switch.** `_phon_is_empty` (`selection.py:2436`) is applied at exactly two sites -- `selection.py:2587` and `selection.py:2725` -- and BOTH are a bare `continue` guarded by nothing. `Selection` (`models.py`) carries no blank/filter/force field among its 15 options (`include_closure`, `enable_overwrite`, `category_scopes`, ... none of them touch this). So `build_full_selection(exclude=frozenset())` -- the maximal "force all" the sweep driver uses -- still drops blanks. **Force-all mode and filtered mode are currently the SAME mode.** The user's two-mode test cannot be run until a switch exists.
  2. **The drop leaves no trace.** Neither site emits a `DroppedItemRecord`, increments a counter, or appends to any tally; the comments say so outright -- *"silently skipped from the inventory"* (`selection.py:2440`) and *"continue  # empty in all fields -- silently skip (FR: dangling)"* (`selection.py:2726`). The filter runs BEFORE the plan exists, so the items never become plan actions and never reach `RunReport.dropped_items`, the channel 024 built precisely so that nothing is dropped silently. Consequence for T024c's live comparison: in `source - after` a filtered blank is **indistinguishable from a real loss**, because no report line explains either.

  **Why this is not cosmetic.** `_phon_is_empty` decides what transfers, and its history on `fullcopy-defects` shows it has been wrong twice in ways that silently changed transfer scope: `4117f26`/`a5772f4` ("content-aware `_phon_is_empty` -- keep items with fields OR children") and `f4e1b83` (phonological rules previewed blank), the latter having *"wrongly dropped every unnamed rule -- making it un-previewable AND silently excluding it from transfer"* per the predicate's own docstring. A drift in this predicate changes what gets copied and NOTHING catches it today.

  **The work, in order:** (a) make the two sites record what they skip -- the existing `DroppedItemRecord` channel is the right home, with a distinct reason token so a filtered blank is never confused with a transfer failure; (b) add the force switch (a `Selection` field, defaulted to today's filtering behaviour so nothing changes silently) so force-all and filtered are genuinely two modes; (c) only then can T024c assert the mode contrast -- filtered `source - after` == the recorded blank-drop set, force-all `source - after` == empty modulo identity substitution. **Sequence (a) before T024c**: without it the live comparison cannot separate filter from defect, which is the whole point of running it - `src/gramtrans/Lib/selection.py`, `src/gramtrans/Lib/models.py`

- [ ] **T025** [US2] Update the quickstart to name `python -m gramtrans.census_cli <subcommand>` throughout, drop the `debug/audit_object_census.py` invocations, and clear the `PLANNED` markers T021/T023 have now satisfied. **Commits to `main`** - `specs/038-transfer-fidelity-gaps/quickstart.md`
...
- [ ] **T037** [P] [US1] Report every natural-key match as such, distinguishable from an identity match (FR-006), and feed `CategoryReport.identity_substitution` - `src/gramtrans/Lib/report.py`
...
- [ ] **T043** [US4] Widen `_plan_gold_reserved_edit` (:194) with an owned-collection pass beside the existing `("Name","Abbreviation","Description")` loop, covering the seven collections. Its two early `Skip(ALREADY_PRESENT_BY_GUID)` returns at :246 and :269 may fire **only** when that pass also finds nothing; otherwise fall through to the existing `PlannedOverwrite(write_mode="merge")` at :298. This widens the existing path rather than adding a parallel enrich path (recorded decision). Update the :219 docstring, which currently says "Owned collections are outside its scope" - `src/gramtrans/Lib/categories.py`
