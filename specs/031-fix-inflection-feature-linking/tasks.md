...
- [ ] T019 [P] [US3] In `tests/unit/test_031_infl_feature_linking.py`: test the diagnosis report shape against a mocked project — `total_features`, `total_values`, `nameless_features`, `nameless_values`, `orphaned_features`, `linked_features`, `feature_name_ws_map`, `duplicate_guid_groups`; assert every feature is classified exactly one of linked/orphaned (contract diagnosis-report COMPLETE).

### Implementation for User Story 3

- [ ] T020 [US3] Implement the read-only report in `debug/diag_infl_features.py` per `contracts/diagnosis-report.md`: walk `MsFeatureSystemOA.FeaturesOC`, count nameless features/values, classify each feature as orphaned vs linked by scanning every `IPartOfSpeech.InflectableFeatsRC`, sample the WS handle carrying a named feature, and detect duplicate-GUID groups. MUST open read-only and perform zero writes (contract READ-ONLY).
- [ ] T021 [US3] Add a guard/assertion path in `debug/diag_infl_features.py` that proves read-only (e.g. no UoW opened; pre/post object-count snapshot equal) and prints the report as plain ASCII (no emoji, per environment rules).
...
- [ ] T022 Run the sweep-pattern audit for the WS-handle-copy bug class (source-handle written into target object): search the other GOLD/feature execute_action paths (e.g. `exception_features`, `inflection_classes`, `variant_types`, `complex_form_types`, `phonological_features`) for the same raw-source-handle string copy; record siblings (file:line + confidence) in a "Pattern audit" section of the PR body.
- [ ] T023 [P] Run the full offline unit suite from the worktree and confirm no regressions vs the merged-tree baseline: `python -m pytest tests/unit -q`.
- [ ] T024 Execute quickstart.md live validation Steps 0–4 against `Ejagham Mini` → a clean/restored `Ejagham Full GT-Test`; attach pre/post diagnosis reports and Import Residue / `[GT-Tag]` evidence. (Destructive Move — attended; restore target from clean backup first.)
- [ ] T025 [P] Update `STATUS.md` with the 031 outcome and evidence links; note prevention-only scope (FR-011) and that the polluted target must be restored out of band.
- [ ] T026 Merge `031-fix-inflection-feature-linking` → `main` after live validation passes; remove the worktree.
