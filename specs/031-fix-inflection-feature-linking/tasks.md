...
- [ ] T024 Execute quickstart.md live validation Steps 0–4 against `Ejagham Mini` → a clean/restored `Ejagham Full GT-Test`; attach pre/post diagnosis reports and Import Residue / `[GT-Tag]` evidence. (Destructive Move — attended; restore target from clean backup first.) **BLOCKED — needs_human**: destructive live write; must be run attended after restoring the target, never under an unattended loop. `Ejagham Full GT-Test` currently reads clean (0 features) per the T020 read-only probe, so it is ready for the attended Move.
