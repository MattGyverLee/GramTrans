...
- [x] T029 [P] Extend `tests/integration/test_conflict_live.py`: UPDATE behavior on Ejagham Mini->Full GT-Test pair (field A diverged takes source; field B target-only preserved); SKIP on all-identical item; OVERWRITE contrast; shim round-trip on a saved selection (SC-002/003/004/005). **DONE 2026-07-13**: scaffold added (collects → 1 skipped, exit 0). Live on target POS 'Adverb': disposition SKIP/UPDATE/OVERWRITE PASS (SC-004); UPDATE non-destructive PASS (SC-002 — Name diverged→source, Description empty-source preserved). **FINDING**: SC-003 destructive-blank does NOT reproduce for multistrings — the fork's `_apply_props_loop` skips empty multistring text (`BaseOperations.py:291 if not text: continue`), so OVERWRITE cannot blank a multistring alt from an empty source (encoded as `xfail(strict)`). See [verification-log.md](./verification-log.md).
