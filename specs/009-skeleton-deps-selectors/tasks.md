...
>
> Skeleton + grammatical-deps selectors shipped: `d9a3372` (skeleton +
> grammatical-deps selectors, preselect-all affixes), `5034db5` (refactor:
> rename dual `_walk_pos` closures, P2-D), `98d8c88` (mark optional slots and
> strike deselected slots in template view); spec/plan/tasks in `60f494a`.
> `build_skeleton_inventory` and `build_deps_inventory` live in
> `src/gramtrans/Lib/selection.py`, covered by
> `tests/unit/test_skeleton_inventory.py` (34 tests) and
> `test_deps_inventory.py` (14 tests). The checkboxes below were reconciled from
...
- [X] T001 Extend `tests/unit/_fakes_affix.py` with fakes for slots (`AffixSlotsOC`,
...
- [X] T002 [P] Unit tests for `build_skeleton_inventory` in `tests/unit/test_skeleton_inventory.py`:
  ...
  empty-POS pruning.
- [X] T003 [P] Unit tests for template-forces-slots + no-affix-re-expansion in
  ...
  either way.
- [X] T004 [P] Unit tests for `build_deps_inventory` in `tests/unit/test_deps_inventory.py`:
...
- [X] T005 [P] Unit tests for EXCLUDED-LOSSY aggregation in
...
- [X] T006 Implement `build_skeleton_inventory(source, affix_picks, target=None)` +
...
- [X] T007 Implement `build_deps_inventory(source, affix_picks, target=None)` + deps dataclasses
...
- [X] T008 Implement the EXCLUDED-LOSSY derivation + aggregation helper (pure) feeding the
  Preview warning channel in `src/gramtrans/Lib/preview.py` / `selection.py`. Makes T005 pass.

## Phase 3: US1 — Preselect affixes (P1)

- [X] T009 [US1] Default every affix row + group tristate to checked in `_PageItemPicker`
  (`src/gramtrans/Lib/ui/selection_wizard.py`); confirm collapse yields all affixes pre-interaction.
- [X] T010 [P] [US1] Unit test preselect-all state -> `affix_picks` == all source affixes.

## Phase 4: US2 — Skeleton page (P1)

- [X] T011 [US2] Add `_PageSkeleton` in `src/gramtrans/Lib/ui/selection_wizard.py`: POS-rooted
...
- [X] T012 [US2] Implement template check/deselect semantics (force full slot set; never
  re-expand affix picks) and collapse of skeleton selection into the plan.
- [X] T013 [P] [US2] Live MCP integration in `tests/integration/test_skeleton_deps_live.py`:
...
- [X] T014 [US3] Add `_PageGramDeps` in `src/gramtrans/Lib/ui/selection_wizard.py`: sections
...
- [X] T015 [P] [US3] Extend live integration: Ejagham deps (0 classes, 0 stem names, features
  0–1 per POS) render preselected without error.

## Phase 6: US4/US5 — Status + closure safety (P1/P2)

- [X] T016 [US4] Ensure target-status column populates on skeleton + deps rows (reuse FR-018
  logic); blank when no target.
- [X] T017 [US5] Wire skeleton/deps deselections into the Preview EXCLUDED-LOSSY warning list
  and the SINGLE consolidated confirm-on-Move dialog (`selection_wizard.py` finish handler +
  `stats_panel`); never per-item prompts.
- [X] T018 [P] [US5] Unit test the Move-gate payload aggregates all omissions into one dialog.

## Phase 7: Sequence + polish

- [X] T019 Rewire the wizard page order to Project+WS → Affixes → Skeleton → Grammatical deps
...
- [X] T020 Apply Layer-1 conflict-mode defaults automatically in the plan build (no UI);
  confirm no ADD/MERGE/OVERWRITE controls remain on selection pages.
- [X] T021 Run `python -m pytest tests/unit -q`; fix any page-index/regression fallout from
  the sequence rewire. Confirm >= prior baseline + new tests.
- [X] T022 Manual UI smoke on Ejagham per quickstart: preselected affixes → skeleton
