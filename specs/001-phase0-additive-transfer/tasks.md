...
## Reconciliation banner (2026-07-13) — this list is a historical artifact

Phase 0 shipped and the codebase advanced through features 002–026. The
architecture the 001 tasks originally assumed (a per-category `CategoryTransfer`
protocol with generic-walker bodies, standalone selection dialogs, a
`FieldWorks.exe -restore` fixture) was **superseded** by the shipped design:
the closure/plan engine in `Lib/preview.py` + `Lib/transfer.py` + `Lib/categories.py`,
the `Lib/ui/selection_wizard.py` wizard, and the headless zip-extract harness in
`tests/integration/harness/`. The remaining unchecked boxes below are therefore
mostly **done-elsewhere / superseded / live-host-gated**, not a live backlog.
Dispositions (inline pointers added to each task):

| Task(s) | Disposition |
|---------|-------------|
| T008 | DONE — PyQt5 chosen and used throughout (`Lib/ui/*`). |
| T014 / T015 | SUPERSEDED — Ejagham Mini used live as `toy_source`; restore is `tests/integration/harness/restore.py` (headless zip extract, replaces the GUI `FieldWorks.exe -restore`). |
| T030–T035 / T033b / T062 | DONE (2026-07-13) — integration scaffolds rewritten to drive the real engine via `tests/integration/harness`; FR-019/FR-020 now run host-free. |
| T038 | DONE — WS materialization runs in `Lib/transfer.py` (`source.WritingSystems.GetAll()`). |
| T039 / T051 | SUPERSEDED — heavy-category transfer is the closure/plan path, not a generic walker. The dead `categories_templates.py` / `categories_msas.py` stubs were **deleted**; `categories_affixes.py` is kept only as the honest placeholder for the flagged item-picks gap (xfail in `test_selection_ui.py`). |
| T075 | SUPERSEDED — selection is `Lib/ui/selection_wizard.py`; the standalone `affix_tree_picker.py` is a pre-wizard relic. |
| T059 / T068 / T077 / T078–T082 / T086 | LIVE-HOST-GATED — require FlexTools + the Ejagham pair. Later features carry their own verification (`test_full_workflow_e2e.py` golden snapshot, `tests/verification/fidelity_census.py`, per-feature `verification-log.md` / `probe-results.md`). |
| T060 / T061 / T069–T071 | LIVE-HOST-GATED — see the live suites (`test_phase3c_affixes_stems_e2e.py`, `test_phonology_live.py`, `test_rules_live.py`). |
...
- [x] T008 [P] Confirm PyQt vs PySide for the host (research.md D3) — DEFERRED to UI implementation start; default PyQt5 per `pyproject.toml`, switch to PySide2 if import fails
...
- [x] T030 [P] [US1] Integration test for full-categories transfer ([US1 Acceptance 1](spec.md)) in `tests/integration/test_e2e_all_categories.py` — runs Move against a freshly-restored `Ejagham Full GT-Test`, snapshots the result via [contracts/run-report.md](contracts/run-report.md), verifies SC-001 timing budget
- [x] T031 [P] [US1] Integration test for pre-existing target objects not modified ([US1 Acceptance 2](spec.md), [SC-004](spec.md)) in `tests/integration/test_target_preserved.py` — pre-snapshot vs post-snapshot diff equals exactly the added items
- [x] T032 [P] [US1] Integration test for WS-mapping required-before-write ([US1 Acceptance 4](spec.md), Scenario E) in `tests/integration/test_ws_mapping_required.py`
- [x] T033 [P] [US1] Integration test for same-source-and-target refusal ([FR-019](spec.md), Scenario D) in `tests/integration/test_same_project_refused.py`
- [x] T033b [P] [US1] **Integration test for target lock / read-only refusal ([FR-020](spec.md))** in `tests/integration/test_target_locked.py` — open the target in FLEx itself or set the project directory read-only; verify the module surfaces the lock condition before any write and aborts. Closes the coverage gap flagged in the 2026-06-19 audit.
- [x] T034 [P] [US1] Integration test asserting GUID preservation per [R6](research.md) on the benchmark fixture in `tests/integration/test_guid_preservation.py`
- [x] T035 [P] [US1] Integration test asserting GOLD inviolability ([FR-022](spec.md)) in `tests/integration/test_gold_inviolable.py` — pre/post snapshot of GOLD objects, asserts byte equality
...
- [x] T038 [P] [US1] Implement WS-creation pre-step in `src/gramtrans/Lib/ws_mapping.py`: materializes `WSMapping.entries[create_in_target=True]` into the target via `project.WritingSystem.Create*` (no separate `categories/writing_systems.py` file under v5.1.0)
