...
- **FR-303**: System MUST preserve GUIDs on transfer for every category in scope when LCM factories support a Guid overload. When a category's factory does NOT support GUID preservation (a flexlibs2 limitation discovered during MCP probing), the planner MUST record the source-GUID → new-GUID mapping in `identity_remap` so downstream references can resolve (same pattern as FR-012).
